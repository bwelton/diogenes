...
#include <map>
extern std::map<uint64_t, std::vector<void *> > MY_CUSTOM_MALLOCMM;
namespace cusp
{
namespace system
{
  ...
  struct my_allocator : thrust::device_malloc_allocator<T>
{
  ...
  {
    void * myPtr = NULL;
    size_t tmp = size_t(n) * sizeof(T);
    //std::cerr << "My size is = " << tmp << std::endl;
    if (MY_CUSTOM_MALLOCMM.find(tmp) != MY_CUSTOM_MALLOCMM.end()){
        if (MY_CUSTOM_MALLOCMM[tmp].size() > 0){
            myPtr = MY_CUSTOM_MALLOCMM[tmp].back();
            MY_CUSTOM_MALLOCMM[tmp].pop_back();
        }
    } else {
      MY_CUSTOM_MALLOCMM[tmp] = std::vector<void *>();
    }

    if (myPtr == NULL) {
	std::cerr << "Allocing " << tmp << std::endl;
        myPtr = thrust::system::cuda::malloc(tmp).get();
    }

    return thrust::device_pointer_cast((T*)myPtr);
  }
  void deallocate(pointer p, size_type n)
  {
    size_t tmp = size_t(n) * sizeof(T);
    MY_CUSTOM_MALLOCMM[tmp].push_back((void *)(p.get()));
  }
};
              ...
              cusp::array1d_format)
{
    ...
    thrust::device_vector<IndexType,my_allocator<IndexType> > rows(A.num_rows);
    thrust::device_vector<ValueType,my_allocator<ValueType> > vals(A.num_rows);


    //cusp::detail::temporary_array<IndexType, DerivedPolicy> rows(exec, A.num_rows);
    //cusp::detail::temporary_array<ValueType, DerivedPolicy> vals(exec, A.num_rows);
    //std::cerr << "My sizes are - " << sizeof(IndexType) << "," << sizeof(ValueType) << "," << A.num_rows << std::endl;
    if(A.num_entries == 0) return;

    typename thrust::device_vector<IndexType>::iterator rows_end;
    typename thrust::device_vector<ValueType>::iterator vals_end;
