...
#include <map>
#include <utility>
std::map<uint64_t, std::pair<uint64_t, void *> > _MyStores;

void * GetMemPointer(uint64_t id, uint64_t size) {
	if(_MyStores.find(id) != _MyStores.end()) {
		if (_MyStores[id].first >= size)
			return _MyStores[id].second;
		cudaFree(_MyStores[id].second);
	}
	void * tmp = NULL;
	fprintf(stderr, "Allocating to id %llu of size %llu\n", id, size);
	cudacall(cudaMalloc(&tmp, size));
	_MyStores[id] = std::make_pair(size, tmp);
	return tmp;
}
#define MY_TIMER_MACRO(X,Y) X;

#define MY_TIMER_MACRO2(X,Y) \
        {\
	...
	fprintf(stderr, "Timer %s took %f time\n", Y, time_span.count()); \
	}
...
//	#ifdef DEBUG
	...
	double t1 = 0;
//	#endif
	csrVal = (float *) GetMemPointer(99, (nnz * sizeof(csrVal[0])));
	csrColIndex =  (int *) GetMemPointer(100, (nnz * sizeof(csrColIndex[0])));
	//csrRowIndex = (int *)  GetMemPointer(1001, (nnz * sizeof(csrColIndex[0])));
	...
	for(int iter = 0; iter < ITERS ; iter ++){
		...
		//if (csrVal22 == NULL) {
		//	csrVal22 = (float *) GetMemPointer(99, (nnz * sizeof(csrVal[0])));
		//cudaMemcpyAsync(csrVal22, csrVal, (size_t ) (nnz * sizeof(csrVal[0])), cudaMemcpyDeviceToDevice);
		//cudaMemcpyAsync(csrColIndex22, csrColIndex,(nnz * sizeof(csrColIndex[0])),cudaMemcpyDeviceToDevice);
		//}
		#ifdef DEBUG
		...
		ythetaT = (float*) GetMemPointer(0,  f * m * sizeof(ythetaT[0]));
		//t0 = seconds();
		//cudacall(cudaMalloc((void** ) &ythetaT, f * m * sizeof(ythetaT[0])));
		//t1 = seconds();
		//fprintf(stderr, "Total Malloc Time %f\n", t1-t0);
		...
		//cudacall(cudaFree(ythetaT)); //FIX 17
...
///*
		...
		#endif
		float * yTX = 0;
		float * yTXT = (float *)GetMemPointer(12345,  f * n * sizeof(yTXT[0]));
		//cudacall(cudaMalloc((void** ) &yTXT, f * n * sizeof(yTXT[0])));
		//t0 = seconds();
		yTX =  (float *) GetMemPointer(1, n * f * sizeof(yTX[0]));
		...
		//cudaDeviceSynchronize(); //FIX 18
		//cudacall(cudaFree(yTX)); //FIX 19
		#ifdef DEBUG
...
//			float * xx =(float *) GetMemPointer(2, f * f * batch_size * sizeof(xx[0]));

			//t0 = seconds();
			#ifdef CUMF_XX_FP16
			MY_TIMER_MACRO(cudaMalloc((void** ) &xx, f/2 * f * batch_size * sizeof(xx[0])), "cudaMalloc XX");
			
			//t1 = seconds();
			MY_TIMER_MACRO(cudacall( cudaMemset(xx, 0, f/2*f*batch_size*sizeof(float)) ), "CUDAMEMSETXX");
			#else
			MY_TIMER_MACRO(cudaMalloc((void** ) &xx, f * f * batch_size * sizeof(xx[0])), "cudaMallocXX");

			//t1 = seconds();
			MY_TIMER_MACRO(cudacall( cudaMemset(xx, 0, f*f*batch_size*sizeof(float)) ), "CUDAMEMSETXX");
			#endif
			//fprintf(stderr, "Total Malloc Time %f\n", t1-t0);
			#ifdef DEBUG
			t1 = seconds();
			...
			#endif
		//MY_TIMER_MACRO(cudaDeviceSynchronize(), "CUDASYNC");
		//MY_TIMER_MACRO(cudaFree(xx),"CUDAFREEXX"); // Fix 22
		}
		//cudacall(cudaFree(yTXT)) // Fix 23
		...
		if (cooRowIndex == NULL) {
			...
			cudacall(cudaMemcpyAsync(cooRowIndex, cooRowIndexHostPtr,(size_t ) (nnz * sizeof(cooRowIndex[0])), cudaMemcpyHostToDevice)); // Fix 24
		}
		//cudacall(cudaMalloc((void** ) &csrColIndex, nnz * sizeof(csrColIndex[0])));
		//cudacall(cudaMalloc((void** ) &csrVal, nnz * sizeof(csrVal[0])));
		//cudacall(cudaMemcpyAsync(csrColIndex, csrColIndexHostPtr,(size_t ) (nnz * sizeof(csrColIndex[0])), cudaMemcpyHostToDevice)); // Fix 25
		//cudacall(cudaMemcpyAsync(csrVal, csrValHostPtr,(size_t ) (nnz * sizeof(csrVal[0])),cudaMemcpyHostToDevice)); // Fix 26

		RMSE<<<(nnz-1)/256 + 1, 256>>>(csrVal, cooRowIndex, csrColIndex, thetaT, XT, errors_train, nnz, error_size, f);
		//cudaDeviceSynchronize(); // Fix 27
		cudaCheckError();
		//cudacall(cudaFree(cooRowIndex)); // Fix 28
		//cudacall(cudaFree(csrColIndex)); // Fix 29
		//cudacall(cudaFree(csrVal)); // Fix 30
